...
class MyTestCase(unittest.TestCase):
    ...
    def testUrlGeneration(self):
        ...
        self.assertEqual("https://api.omgwtfnzbs.me/xml/?api=apikey&user=anuser&search=aquery", urls[0])

        self.args = SearchRequest(query="aquery", category=getCategoryByAnyInput("tvhd"))
        urls = self.omgwtf.get_search_urls(self.args)
        self.assertEqual(1, len(urls))
        self.assertEqual("https://api.omgwtfnzbs.me/xml/?api=apikey&user=anuser&search=aquery&catid=20", urls[0])

        self.args = SearchRequest(query="aquery", maxage=100)
        urls = self.omgwtf.get_search_urls(self.args)
        self.assertEqual(1, len(urls))
        self.assertEqual("https://api.omgwtfnzbs.me/xml/?api=apikey&user=anuser&search=aquery&retention=100", urls[0])

        self.args = SearchRequest(query="aquery", category=getCategoryByAnyInput("tvhd"), maxage=100)
        urls = self.omgwtf.get_search_urls(self.args)
        self.assertEqual(1, len(urls))
        self.assertEqual("https://api.omgwtfnzbs.me/xml/?api=apikey&user=anuser&search=aquery&retention=100&catid=20", urls[0])

        self.args = SearchRequest()
        urls = self.omgwtf.get_search_urls(self.args)
        self.assertEqual(1, len(urls))
        self.assertEqual("https://rss.omgwtfnzbs.me/rss-download.php?api=apikey&user=anuser", urls[0])
        
        self.args = SearchRequest(category=getCategoryByAnyInput("tvhd"))
        urls = self.omgwtf.get_search_urls(self.args)
        self.assertEqual(1, len(urls))
        self.assertEqual("https://rss.omgwtfnzbs.me/rss-download.php?api=apikey&user=anuser&catid=20", urls[0])
    
    def testGetShowSearchUrls(self):
        self.args = SearchRequest(query="aquery")
        urls = self.omgwtf.get_showsearch_urls(self.args)
        self.assertEqual(1, len(urls))
        self.assertEqual("https://api.omgwtfnzbs.me/xml/?api=apikey&user=anuser&search=aquery&catid=19,20,21", urls[0])
        ...
        urls = self.omgwtf.get_showsearch_urls(self.args)
        self.assertEqual(1, len(urls))
        self.assertEqual("https://api.omgwtfnzbs.me/xml/?api=apikey&user=anuser&search=aquery+s01&catid=19,20,21", urls[0])

        self.args = SearchRequest(query="aquery", season=1, episode=2)
        urls = self.omgwtf.get_showsearch_urls(self.args)
        self.assertEqual(1, len(urls))
        self.assertEqual("https://api.omgwtfnzbs.me/xml/?api=apikey&user=anuser&search=aquery+s01e02&catid=19,20,21", urls[0])

        self.args = SearchRequest()
        urls = self.omgwtf.get_showsearch_urls(self.args)
        self.assertEqual(1, len(urls))
        self.assertEqual("https://rss.omgwtfnzbs.me/rss-download.php?api=apikey&user=anuser&catid=19,20,21", urls[0])
    ...
    def testGetMovieSearchUrls(self):
        ...
        self.assertEqual(1, len(urls))
        self.assertEqual("https://api.omgwtfnzbs.me/xml/?api=apikey&user=anuser&search=tt0169547&catid=15,16,17,18", urls[0])

        self.args = SearchRequest(identifier_key="tmdb", identifier_value="14", category=getCategoryByAnyInput("movieshd"))
        urls = self.omgwtf.get_moviesearch_urls(self.args)
        self.assertEqual(1, len(urls))
        self.assertEqual("https://api.omgwtfnzbs.me/xml/?api=apikey&user=anuser&search=tt0169547&catid=16", urls[0])
    ...
    def testProcessSearchResults(self):
        with open("mock/omgwtf_search.xml", encoding="latin-1") as f:
            ...
            self.assertEqual("https://api.omgwtfnzbs.me/nzb/?id=x30FI&user=auser&api=apikey", entries[0].link)
            ...
            self.assertEqual("https://omgwtfnzbs.me/details?id=x30FI", entries[0].details_link)
    ...
    def testGetNzbLink(self):
        ...
        self.assertEqual("https://api.omgwtfnzbs.me/nzb?api=apikey&id=guid&user=anuser", link)
